...
for ($i = 1; $i <= 12; $i++) {
	$stid = oci_parse($conn, "SELECT SUM(AMOUNT) AS AMT FROM TRANSACTION WHERE  EXTRACT(MONTH FROM DOT)='$i' AND CATEGORY='Travel'");
	$r = oci_execute($stid);
	if (!$r) {
		$e = oci_error($stid);
		trigger_error(htmlentities($e['message'], ENT_QUOTES), E_USER_ERROR);
	...
	$travel[$i]+=$row->AMT;
	//
	$stid1 = oci_parse($conn, "SELECT SUM(AMOUNT) AS AMT FROM TRANSACTION WHERE EXTRACT(MONTH FROM DOT)='$i' AND CATEGORY='Medical'");
	...
	$medical[$i]+=$row1->AMT;
	//
	$stid2 = oci_parse($conn, "SELECT SUM(AMOUNT) AS AMT FROM TRANSACTION WHERE  EXTRACT(MONTH FROM DOT)='$i' AND CATEGORY='Restaurants'");
	...
	$rest[$i]+=$row2->AMT;
	//
	$stid3 = oci_parse($conn, "SELECT SUM(AMOUNT) AS AMT FROM TRANSACTION WHERE  EXTRACT(MONTH FROM DOT)='$i' AND CATEGORY='Groceries'");
	...
	$groc[$i]+=$row3->AMT;
	//
	$stid4 = oci_parse($conn, "SELECT SUM(AMOUNT) AS AMT FROM TRANSACTION WHERE  EXTRACT(MONTH FROM DOT)='$i' AND CATEGORY='Household'");
	...
	$house[$i]+=$row4->AMT;
	//
	$stid5 = oci_parse($conn, "SELECT SUM(AMOUNT) AS AMT FROM TRANSACTION WHERE  EXTRACT(MONTH FROM DOT)='$i' AND CATEGORY='Education'");
	...
	$edu[$i]+=$row5->AMT;
	//
	$stid6 = oci_parse($conn, "SELECT SUM(AMOUNT) AS AMT FROM TRANSACTION WHERE  EXTRACT(MONTH FROM DOT)='$i' AND CATEGORY='Fuel'");
	...
	$fuel[$i]+=$row6->AMT;
	//
	$stid7 = oci_parse($conn, "SELECT SUM(AMOUNT) AS AMT FROM TRANSACTION WHERE  EXTRACT(MONTH FROM DOT)='$i' AND CATEGORY='Rent'");
...
<head>
	...
	<title>Admin Monthly Analysis</title>
