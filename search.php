...
if($transactionid != NULL){
	$stid1 = oci_parse($conn, "SELECT * FROM TRANSACTION WHERE TRANSACTIONID='$transactionid' AND CUSTOMERID='$customerid'");
}
else{
	if($idate != NULL && $fdate != NULL){
		if($category != NULL){
			$stid1 = oci_parse($conn, "SELECT * FROM TRANSACTION WHERE CATEGORY='$category' AND DOT BETWEEN '$idate' AND '$fdate' AND CUSTOMERID='$customerid'");
		}
		else{
			$stid1 = oci_parse($conn, "SELECT * FROM TRANSACTION WHERE DOT BETWEEN '$idate' AND '$fdate' AND CUSTOMERID='$customerid'");
		}
	}
	else{
		if($category != NULL){
